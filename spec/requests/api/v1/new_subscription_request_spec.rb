...
RSpec.describe 'New Tea Subscription Request' do
  describe 'request response path' do
    it 'can create a new tea subscription' do
      ...
      subsciption1 = Subscription.create!(title: "Monthly", price: 10.00)
      ...
      expect(customer_subscription[:"Subscription Data"]).to have_key :customer_subscription
    ...
    it 'shows data from response' do
      customer1 = Customer.create!(first_name: "Billy", last_name: "Bob", email: "billybobsemail@email.com", address: "123 billy bob lane")
      subscription1 = Subscription.create!(title: "Bobs and Weaves", price: 10.00)
      post "/api/v1/customers/#{customer1.id}/subscriptions", params: {customer_id: customer1.id, subscription_id: subscription1.id, status: 0, frequency: "Monthly"}
      customer_subscription = JSON.parse(response.body, symbolize_names: true)[:"Subscription Data"]

      expect(customer_subscription[:customer_subscription][:customer_id]).to eq customer1.id
      expect(customer_subscription[:customer_subscription][:subscription_id]).to eq subscription1.id
